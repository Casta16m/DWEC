...
// ---------------------Examen-------------------- //
function menuPoblacions() {
    let pobles = ["Girona", "Madrid", "Torello"];
    pobles.sort();
    const select = document.getElementsByTagName("select")[1];

    for (let i = 0; i < pobles.length; i++) {
        let option = document.createElement("option");
        option.value = pobles[i];
        option.innerHTML = pobles[i];
        select.append(option);
    }
}

function comprovaEdifici(Edifici) {
    const regExp = /^[a-z]{5}\+[0-9]{3}\.[a-z]{2}$/;

    return regExp.test(Edifici);
}

function comprovarImatge() {
    let validarImatge = document.getElementsByClassName('Localitzacio')[0];

    validarImatge.addEventListener('change', validarEdifici);
}

function validarEdifici() {

    const Loc = document.getElementsByClassName('Localitzacio')[0];
    let img = document.getElementsByClassName('imagen')[4];
    let Edifici = Loc.value;

    if (comprovaEdifici(Edifici) == true) {
